            ...
            if self.destinationLocation != nil {
                self.navigationService.getDirections(destinationLocation: self.destinationLocation, request: MKDirectionsRequest()) { steps in
                    for step in steps {
                        ...
                        let locations = step.getLocation()
                        
                        for var i: Int in 0 ..< locations.count
                        {
                            if i == locations.count - 1
                            {
                                self.annotations.append(POIAnnotation(coordinate: locations[i].coordinate, name: "N " + step.instruction))
                            }
                            else
                            {
                                self.annotations.append(POIAnnotation(coordinate: locations[i].coordinate, name: "N "))
                            }
                        }
                        
//                        self.annotations.append(POIAnnotation(coordinate: step.getLocation().coordinate, name: "N " + step.instruction))
                    }
    ...
    private func setTripLegFromStep(_ tripStep: routeStep, and index: Int) {
        if index > 0 {
    ...
    private func getTripLeg(for index: Int, and tripStep: routeStep) {
        let previousIndex = index - 1
        let previousStep = steps[previousIndex]
        let locations = previousStep.getLocation()
        
        let intermediaryLocs = previousStep.getLocation()
        var intermediarySteps: [CLLocationCoordinate2D] = []
        
        for var i: Int in 1 ..< intermediaryLocs.count
        {
            let previousLocation = intermediaryLocs[i - 1]
            let nextLocation = intermediaryLocs[i]
            
            let intermediarySteps1 = CLLocationCoordinate2D.getIntermediaryLocations(currentLocation: previousLocation, destinationLocation: nextLocation)

            intermediarySteps.append(contentsOf: intermediarySteps1)
        }
    ...
    private func getInitialLeg(for tripStep: routeStep) {
