...
extension ViewController: MessagePresenting {
    ...
    private func addSphere(for step: routeStep) {
        let stepLocation = step.getLocation()
        
        for var i: Int in 0 ..< stepLocation.count
        {
            let stepLoc = stepLocation[i]
            var sphere = BaseNode(title: step.instruction, location: stepLoc)
            
            if i == stepLocation.count - 1
            {
                sphere = BaseNode(title: step.instruction, location: stepLoc)
                sphere.addNode(with: 1.0, and: .green, and: step.instruction)
            }
            else
            {
                sphere = BaseNode(title: "", location: stepLoc)
                sphere.addNode(with: 1.0, and: .green, and: "")
            }
            
            let locationTransform = MatrixHelper.transformMatrix(for: matrix_identity_float4x4, originLocation: startingLocation, location: stepLoc)
            let stepAnchor = ARAnchor(transform: locationTransform)
//            let sphere = BaseNode(title: step.instruction, location: stepLoc)
            anchors.append(stepAnchor)
            sphere.location = stepLoc
...
extension ViewController:  Mapable {
    ...
    private func addAnchors(steps: [routeStep]) {
